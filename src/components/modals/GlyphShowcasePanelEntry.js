...
default {
  ...
  computed: {
    ...
    levelText() {
      ...
      const arrow = this.isLevelCapped ? "<i class='fas fa-sort-down'></i>" : (this.isLevelBoosted ? "<i class='fas fa-sort-up'></i>" : "");
      // eslint-disable-next-line no-nested-ternary
...
${arrow}${formatInt(this.effectiveLevel)}${arrow}
    ...
    rarityStyle() {
      ...
      const color = CosmeticGlyphTypes[this.glyph.type].ignoreRarityColor ? GlyphAppearanceHandler.getBorderColor(this.type) : getRarity(this.glyph.strength)[Theme.current()
    ...
    glyphEffectList() {
      ...
      const effects = getGlyphEffectValuesFromBitmask(this.glyph.effects, this.effectiveLevel, this.glyph.strength, this.type)
  ...
  methods: {
    ...
    formatEffectString(dbEntry, value) {
      ...
      return {
        text: `${rawDesc}`.replace("{value}", singleValue)
    ...
    clickGlyph(glyph) {
      if (Glyphs.isMusicGlyph(glyph)) {
        new Audio(`audio/note${GLYPH_TYPES.indexOf(glyph.type) + 1}.mp3`)
          .play();
      }
    },
  },

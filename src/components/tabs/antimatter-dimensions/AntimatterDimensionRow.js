...
export default {
  ...
  computed: {
    ...
    name() {
      return `${AntimatterDimension(this.tier).shortDisplayName} Dimension`;
    },
    costDisplay() {
  ...
  methods: {
    update() {
      ...
      const buyUntil10 = true;
      this.isCapped = tier === 8 && Enslaved.isRunning && dimension.bought >= 1;
      this.multiplier.copyFrom(AntimatterDimension(tier)
        .multiplier);
      ...
      this.isCostsAD = NormalChallenge(6)
        .isRunning && tier > 2 && !this.isContinuumActive;
...
<div
class="fill-possible"
:style="{ 'width': (boughtBefore10 + howManyCanBuy)*10 + '%' }"
/>
<div

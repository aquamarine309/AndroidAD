export default {
  ...
  computed: {
    ...
    dimName() {
      return AntimatterDimension(this.requirement.tier)
        .shortDisplayName;
    },
    buttonText() {
      ...
      const reset = [];
      if (!Achievement(111)
        .isUnlocked) reset.push("Dimensions");
      if (!Achievement(143)
        .isUnlocked) reset.push("Dimension Boosts");
    ...
    sumText() {
      ...
      const sum = parts.map(this.formatGalaxies)
        .join(" + ");
      if (parts.length >= 2) {
...
.join(", ").capitalize()}`;
  ...
  methods: {
    update() {
      ...
      this.distantStart = EternityChallenge(5)
        .isRunning ? 0 : Galaxy.costScalingStart;
  ...
  template: `<div class="reset-container galaxy">

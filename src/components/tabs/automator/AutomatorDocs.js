...
default {
  ...
  computed: {
    ...
    currentScript() {
      return CodeMirror.Doc(this.currentScriptContent, "automato")
        .getValue();
    },
    errorStyle() {
      return {
    ...
    currentEditorScriptName() {
      return this.scripts.find(s => s.id === this.currentScriptID)
        .name;
    },
  },
  watch: {
  ...
  methods: {
    ...
    updateScriptList() {
      this.scripts = Object.values(player.reality.automator.scripts)
        .map(script => ({
        id: script.id,
        name: script.name,
      }));

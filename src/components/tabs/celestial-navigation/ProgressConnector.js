export default {
  ...
  computed: {
    ...
    completePath() {
      ...
      const insetPath = this.getOffsetPath(-w / 2, tStart, tEnd)
        .transformedBy(tform);
      const outsetPath = this.getOffsetPath(w / 2, tEnd, tStart)
        .transformedBy(tform);
      ...
      const capCP0 = inEnd.position(1)
        .plus(endVector.times(w / 2));
      const capCP1 = outStart.position(0)
        .plus(endVector.times(w / 2));
  ...
  methods: {
    generateIncompletePath(tStart, tEnd) {
      ...
      const tform = AffineTransform.translation(s0.position.negative)
        .rotated(-s0.direction.angle);
      return inset.transformedBy(tform)
        .toSVG("M") + outset.transformedBy(tform)
        .toSVG("L");
    },

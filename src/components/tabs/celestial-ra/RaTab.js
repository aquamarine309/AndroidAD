...
export default {
  ...
  computed: {
    ...
    pets: () => [{
      ...
      pet: Ra.pets.effarig,
      scalingUpgradeVisible: () => AlchemyResources.all.filter(r => r.isUnlocked)
        .length > 0,
      scalingUpgradeText: () => {
        const resources = AlchemyResources.all.filter(r => r.isUnlocked)
          .length;
...
+${formatInt(Ra.unlocks.improvedStoredTime.effects.realTimeCap.effectOrDefault(0) / (1000 * 3600))} hours`,
    ...
    petStyle() {
      return {
        color: (this.petWithRemembrance === "") ? "white" : this.pets.find(pet => pet.pet.name === this.petWithRemembrance)
          .pet.color,
      };
    },
    ...
    runDescription() {
      return GameDatabase.celestials.descriptions[4].effects()
...
will be generated based on certain resource amounts.`;
  ...
  template: `<div class="l-ra-celestial-tab">
...
Click the icons below the bar to see descriptions of upgrades,
<br>
and click <i class="fas fa-question-circle" /> icons for specific resource information.
</div>

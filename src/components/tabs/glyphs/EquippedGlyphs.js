...
default {
  ...
  computed: {
    ...
    undoTooltip() {
      ...
      return this.undoAvailable ? ("Unequip the last equipped Glyph and rewind Reality to when you equipped it." +
        " (Most resources will be fully reset)") : "Undo is only available for Glyphs equipped during this Reality";
    },
    unequipText() {
    ...
    glyphRespecStyle() {
      if (this.respec) {
        return {
          "background-color": "var(--color-good--option)",
          color: "black",
          cursor: "pointer",
        };
      }
  ...
  methods: {
    ...
    clickGlyph(glyph, idx, increaseSound = false) {
      if (Glyphs.isMusicGlyph(glyph)) {
        ...
        new Audio(`audio/note${sound}.mp3`)
          .play();
      }
    }
  },

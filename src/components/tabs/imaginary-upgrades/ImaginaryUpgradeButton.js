...
export default {
  ...
  watch: {
    isAutobuyerOn(newValue) {
      Autobuyer.imaginaryUpgrade(this.upgrade.id)
        .isActive = newValue;
    }
  },
  methods: {
    update() {
      ...
      this.isAutoUnlocked = ImaginaryUpgrade(20)
        .canBeApplied;
      this.canBeLocked = upgrade.config.canLock && !this.isAvailableForPurchase;
      this.hasRequirementLock = upgrade.hasPlayerLock;
      if (this.isRebuyable) this.isAutobuyerOn = Autobuyer.imaginaryUpgrade(upgrade.id)
        .isActive;
    ...
    getETAText() {
      ...
      if (isFinite(time)) return TimeSpan.fromSeconds(time)
        .toString();

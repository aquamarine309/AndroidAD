...
export const PerkNetwork = {
  ...
  initializeIfNeeded() {
    ...
    this.network.on("dragging", () => {
      SecretAchievement(45)
        .tryUnlock();
      if (Theme.current()
        .name === "S4") PerkNetwork.updatePerkSize();
    });
  ...
  makeNetwork() {
    ...
    this.nodes = new DataSet(Perks.all.map(perk => ({
      ...
      title: (isDisabled(perk) ? htmlTitle(
        `<span style='text-decoration: line-through;'>${perk.config.description}</span>`) : `${perk.config.description} ${perk.config.automatorPoints && !isDisabled(perk)
? `(+${formatInt(perk.config.automatorPoints)} AP)`
: ""}`),
      x: selectPos(perk.config)
        .x,
      y: selectPos(perk.config)
        .y,
    })));
  ...
  moveToDefaultLayoutPositions(layoutIndex) {
    ...
    for (const key of Object.keys(PerkNetwork.network.getPositions())) {
      const id = Number(key);
      const config = Perks.all.find(p => p.id === id)
        .config;
  ...
  resetPosition(centerOnStart) {
    const center = centerOnStart ? PerkNetwork.network.body.nodes[GameDatabase.reality.perks.firstPerk.id] : (PerkLayouts[player.options.perkLayout].centerOffset ?? new Vector(0, 0));
    this.network.moveTo({
      position: {
        x: center.x,
        y: center.y
  ...
  updatePerkSize() {
    function nodeSize(perk) {
      ...
      const mod = Theme.current()
        .name === "S4" ? 10 * Math.sin(5 * PerkNetwork.pulseTimer + 0.1 * perk._config.id) : 0;

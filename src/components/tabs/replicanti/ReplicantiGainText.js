export default {
  ...
  methods: {
    update() {
      ...
      const log10GainFactorPerTickUncapped = Decimal.divide(getGameSpeedupForDisplay() * updateRateMs * (Math.log(player.replicanti.chance + 1)), getReplicantiInterval(false))
        .dividedBy(Math.LN10);
      ...
      if (isAbove308) {
        ...
        const gainFactorPerSecond = logGainFactorPerTick.times(postScale)
          .plus(1)
        ...
        const timeToThousand = coeff.times(nextMilestone.divide(replicantiAmount)
          .pow(postScale)
          .minus(1));
      ...
      if (RealityUpgrade(6)
        .isBought && !Pelle.isDoomed) {
        baseGalaxiesPerSecond = galaxiesPerSecond.divide(RealityUpgrade(6)
          .effectValue);
...
${TimeSpan.fromSeconds(secondsPerGalaxy.toNumber())}`;
          ...
          const allGalaxyTime = Decimal.divide(effectiveMaxRG - effectiveCurrentRG, baseGalaxiesPerSecond)
            .toNumber();
...
${TimeSpan.fromSeconds(Math.clampMin(allGalaxyTime - thisGalaxyTime, 0))})`;
  ...
  template: `<p class="c-info-color">{{ remainingTimeText }}<br>{{ galaxyText }}</p>`
}
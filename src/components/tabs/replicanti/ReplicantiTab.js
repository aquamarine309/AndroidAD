...
from "./ReplicantiUpgradeButton.js";
...
export default {
  ...
  computed: {
    ...
    replicantiChanceSetup() {
      ...
      value => `Chance: ${formatPercents(value)}`,
    ...
    replicantiIntervalSetup() {
      ...
      function formatInterval(interval) {
        ...
        if (
        Number.isFinite(intervalNum) && intervalNum > 1 && upgrade.isCapped) {
          // Checking isCapped() prevents text overflow when formatted as "__ ➜ __"
          return TimeSpan.fromMilliseconds(intervalNum)
            .toStringShort(false);
        }
...
multiplier to Infinity Points from Glyph Alchemy`);
    ...
    toMaxTooltip() {
      ...
      return this.estimateToMax.lt(0.01) ? "Currently Increasing" : TimeSpan.fromSeconds(this.estimateToMax.toNumber())
        .toStringShort();
    }
  },
  methods: {
    update() {
      ...
      this.unlockCost = new Decimal(1e140)
      ...
      this.isInEC8 = EternityChallenge(8)
        .isRunning;
      if (this.isInEC8) {
      ...
      Decimal.log10(Replicanti.amount) * getAdjustedGlyphEffect("replicationdtgain"),
      1);
      ...
      if (this.hasRaisedCap) {
        ...
        this.capMultText = TimeStudy(31)
          .canBeApplied ? `Base: ${formatX(mult.pow(1 / TimeStudy(31).effectValue), 2)}; after TS31: ${formatX(mult, 2)}` : formatX(mult, 2);
      }
    ...
    calculateEstimate() {
      ...
      return coeff.times(nextMilestone.divide(this.amount)
        .pow(postScale)
        .minus(1));
    }
  },
  ...
  <div class="l-replicanti-tab">
    ...
    <template v-else>
      ...
      <p class="c-replicanti-description">
        You have
        <span class="c-replicanti-description__accent c-replicanti-amount">{{ format(amount, 2, 0) }}</span>
        Replicanti, translated to
        <br>
        <span v-html="boostText" class="c-replicanti-boost" />
      </p>
      <div
        v-if="hasMaxText"

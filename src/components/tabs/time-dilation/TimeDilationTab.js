...
export default {
  ...
  computed: {
    ...
    allRebuyables() {
      ...
      const result = [];
      if (upgradeRows.length % 2 !== 0) {
        result.push([upgradeRows[0]]);
        upgradeRows.shift();
      }
      for (let i = 0; i < upgradeRows.length; i += 2) {
        result.push(upgradeRows.slice(i,  i + 2));
      }
      return result;
    },
    allSingleUpgrades() {
  ...
  methods: {
    update() {
      ...
      if (PelleRifts.paradox.isActive) {
        ...
        const drainPerSecond = this.dilatedTime.add(rawDTGain.times(tickProp))
  ...
  template: `
  <div class="l-dilation-tab c-info-color">
    <span>
      You have

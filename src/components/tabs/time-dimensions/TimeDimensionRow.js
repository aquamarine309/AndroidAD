...
import AndroidTooltip from "../../AndroidTooltip.js";

export default {
  name: "TimeDimensionRow",
  components: {
  ...
  computed: {
    ...
    timeEstimate() {
      ...
      const time = Decimal.sub(this.ttCost, this.currTT)
        .dividedBy(this.ttGen);
      return time.gt(0) ? `Enough TT in ${TimeSpan.fromSeconds(time.toNumber()).toStringShort()}` : "";
    },
    costTooltip() {
        return this.showCostTitle ? `Cost: ${format(this.cost)} EP` : `${format(this.cost)} EP`;
    }
  },
  watch: {
    isAutobuyerOn(newValue) {
      Autobuyer.timeDimension(this.tier)
        .isActive = newValue;
    }
  },
  methods: {
    update() {
      ...
      this.isAutobuyerOn = Autobuyer.timeDimension(this.tier)
        .isActive;
      this.realityUnlocked = PlayerProgress.realityUnlocked();
      this.showTTCost = !this.isUnlocked && !this.shiftDown;
      if (this.tier > 4) this.ttCost = TimeStudy.timeDimension(this.tier)
        .cost;
      this.currTT.copyFrom(Currency.timeTheorems.value);
      this.ttGen.copyFrom(getTTPerSecond()
        .times(getGameSpeedupFactor()));
    },
    buyTimeDimension() {
      if (!this.isUnlocked) {
        TimeDimension(this.tier)
          .tryUnlock();
        return;
      }
  ...
  <GenericDimensionRowText
    ...
    :rate="rateOfChange"
  />
  <div class="l-dim-row-multi-button-container c-dim-tooltip-container">
    <AndroidTooltip
      :text="costTooltip"
      :condition="!isUnlocked && !requirementReached"
    >
      <PrimaryButton

import TimeDimensionRow from "./TimeDimensionRow.js";
...
export default {
  ...
  computed: {
    costIncreases: () => TimeDimension(1)
      .costIncreaseThresholds,
  },
  methods: {
    update() {
      this.showLockedDimCostNote = !TimeDimension(8)
        .isUnlocked && player.realities >= 1;
      ...
      this.upgradeThreshold.copyFrom(FreeTickspeed.fromShards(Currency.timeShards.value)
        .nextShards);
      this.shardsPerSecond.copyFrom(TimeDimension(1)
        .productionPerSecond);
      this.incomeType = EternityChallenge(7)
        .isRunning ? "Eighth Infinity Dimensions" : "Time Shards";
      this.areAutobuyersUnlocked = Autobuyer.timeDimension(1)
        .isUnlocked;
    },
    maxAll() {
  ...
  template: `
  <div class="l-time-dim-tab l-centered-vertical-tab c-info-color">
    ...
    <div>
      <p>
        You have gained
        <span class="c-time-dim-description__accent">{{ formatInt(totalUpgrades) }}</span> Tickspeed upgrades.
        <br>
        You have <span class="c-time-dim-description__accent">{{ format(timeShards, 2, 1) }}</span> Time Shards.
      </p>
      <p>

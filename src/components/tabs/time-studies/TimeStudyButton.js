...
export default {
  ...
  methods: {
    update() {
      ...
      this.eternityChallengeRunning = study.type === TIME_STUDY_TYPE.ETERNITY_CHALLENGE && EternityChallenge.current?.id === study.id;
      if (!this.isBought) {
        this.isAvailableForPurchase = study.canBeBought && study.isAffordable;
      }
      this.STCost = this.study.STCost;
      this.isCompleteEC = this.study.type === TIME_STUDY_TYPE.ETERNITY_CHALLENGE && EternityChallenge(this.study.id)
    ...
    handleTouchStart() {
      this.timer = setTimeout(this.shiftClick, 600);
    },
    handleTouchMove() {

...
const powerArray = [];
if (isC2Running) powerArray.push(`Normal Challenge 2: ${formatPercents(player.chall2Pow, 2, 2)}`);
if (isC3Running) powerArray.push(`Normal Challenge 3: ${formatX(player.chall3Pow, 3, 4)}`);
if (isIC6Running) powerArray.push(`Infinity Challenge 6: /
${format(new Decimal(1).timesEffectOf(InfinityChallenge(6)), 2, 2)}`);
if (isIC8Running) powerArray.push(`Infinity Challenge 8: /
${format(new Decimal(1).timesEffectOf(InfinityChallenge(8)).reciprocal(), 2, 2)}`);
this.challengePower = powerArray.join(", ");
}
},
},
template: `<div class="c-info-color">

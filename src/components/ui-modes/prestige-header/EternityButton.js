...
default {
  ...
  computed: {
    ...
    tachyonAmountStyle() {
      ...
      if (this.currentTachyons.eq(0)) {
        ...
        ratio = this.gainedTachyons.div(this.currentTachyons)
          .toNumber();
      }
  ...
  methods: {
    update() {
      this.isVisible = Player.canEternity || EternityMilestone.autoUnlockID.isReached || InfinityDimension(8)
        .isUnlocked;
      ...
      TimeSpan.fromMilliseconds(player.records.thisEternity.realTime)
        .totalMinutes));
...
Eternity for
<span :style="amountStyle" class="o-prestige-header-currency">{{ format(gainedEP, 2) }}</span>
...
Eternity for <span :style="tachyonAmountStyle" class="o-prestige-header-currency">{{ format(gainedTachyons, 2, 1) }}</span>
...
Eternity for <span :style="amountStyle" class="o-prestige-header-currency">{{ format(gainedEP, 2, 2) }}</span> EP
</template>
<template v-else>
Eternity for <span :style="tachyonAmountStyle" class="o-prestige-header-currency">{{ format(gainedTachyons, 2, 1) }}</span> TP
</template>
<br>
